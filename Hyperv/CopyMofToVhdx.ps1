...
ForEach ($vm in $vms) {
    if (($vm.state) -eq 'Off') {
        ...
        if (($vm.name) -eq 'POSHCL1') {
            copy-item "$((get-item env:userprofile).value)\Documenten\GitHub\DSCLabCoen\TestW10Enterprise.xml" -Destination "$Driveletter\Unattend.xml"
        }
        if (($vm.name) -eq 'POSHFS') {
            New-item -path "$Driveletter\ConfigFiles" -itemType Directory
            Copy-item -path "$((get-item env:userprofile).value)\Documenten\GitHub\DSCLabCoen\routing.ps1" "$Driveletter\ConfigFiles\routing.ps1"
        }
        foreach ($module in $modules) {
